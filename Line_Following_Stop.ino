...
void loop() {
  ...
  if (prevLeft == LOW && curLeft == HIGH) leftCount++;
  prevLeft = curLeft;

  int curRight = digitalRead(RIGHT_D0);
  if (prevRight == LOW && curRight == HIGH) rightCount++;
  prevRight = curRight;

  float leftDist  = leftCount * DIST_PER_COUNT;
  float rightDist = rightCount * DIST_PER_COUNT;
